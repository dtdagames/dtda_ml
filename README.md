...
One consequence is worth spelling out for anyone who extended a model before this release, because nothing in the code says it: the library now calls the plain names among themselves, and dynamic dispatch follows the name that is called. Measured on `DTDAKMeans`: `fit_predict()` calls `predict()`, so a subclass that overrides `_predict()` gets the base grouping back and its override never runs. Move the override to `predict()` and the two spellings agree again, the internal call reaching it because it calls `predict()`, and a caller that still writes `model._predict(X)` reaching it too, the older name forwarding to the overridden one. The same was measured on three more pairs, the underscored override passed over each time and reached again once moved to the plain name: `save()` reaching `to_dict()` and `load()` reaching `from_dict()`, which live in `DTDATools` and so behave that way on every model, and `DTDAScaler.fit_transform()` reaching `fit()` and `transform()`, measured on that class alone. Those are the pairs checked; wherever else the library calls a plain name among itself, expect the same and verify it rather than assume either way.
